...
int testManagedGateway_sendEvent(void)
{
    ...
    TEST_ASSERT("IoTPManagedGateway_sendEvent: Invalid managedGateway object", rc == IOTPRC_ARGS_NULL_VALUE, "rcE=%d rcA=%d", IOTPRC_ARGS_NULL_VALUE, rc);
    rc = IoTPConfig_create(&config, "./wiotpgw.yaml");
    TEST_ASSERT("IoTPManagedGateway_sendEvent: Create config object", rc == IOTPRC_SUCCESS, "rcE=%d rcA=%d", IOTPRC_SUCCESS, rc);
    rc = IoTPManagedGateway_create(&managedGateway, config);
    TEST_ASSERT("IoTPManagedGateway_sendEvent: Create managedGateway with valid config", rc == IOTPRC_SUCCESS, "rcE=%d rcA=%d", IOTPRC_SUCCESS, rc);
    rc = IoTPManagedGateway_sendEvent(managedGateway, NULL, NULL, NULL, 0, NULL);
    TEST_ASSERT("IoTPManagedGateway_sendEvent: Invalid event ID", rc == IOTPRC_ARGS_NULL_VALUE, "rcE=%d rcA=%d", IOTPRC_ARGS_NULL_VALUE, rc);
    rc = IoTPManagedGateway_sendEvent(managedGateway, "status", NULL, NULL, 0, NULL);
    TEST_ASSERT("IoTPManagedGateway_sendEvent: Invalid format", rc == IOTPRC_ARGS_NULL_VALUE, "rcE=%d rcA=%d", IOTPRC_ARGS_NULL_VALUE, rc);
    rc = IoTPManagedGateway_sendEvent(managedGateway, "status", NULL, "json", -1, NULL);
    TEST_ASSERT("IoTPManagedGateway_sendEvent: Invalid QoS=-1", rc == IOTPRC_ARGS_INVALID_VALUE, "rcE=%d rcA=%d", IOTPRC_ARGS_INVALID_VALUE, rc);
    rc = IoTPManagedGateway_sendEvent(managedGateway, "status", NULL, "json", 3, NULL);
    TEST_ASSERT("IoTPManagedGateway_sendEvent: Invalid QoS=3", rc == IOTPRC_ARGS_INVALID_VALUE, "rcE=%d rcA=%d", IOTPRC_ARGS_INVALID_VALUE, rc);
    rc = IoTPManagedGateway_destroy(managedGateway);
    TEST_ASSERT("IoTPManagedGateway_sendEvent: Destroy a valid managedGateway handle", rc == IOTPRC_SUCCESS, "rcE=%d rcA=%d", IOTPRC_SUCCESS, rc);
    rc = IoTPConfig_clear(config);
    TEST_ASSERT("IoTPManagedGateway_sendEvent: Clear Config", rc == IOTPRC_SUCCESS, "rcE=%d rcA=%d", IOTPRC_SUCCESS, rc);
    return rc;
}
...
{
    int rc = 0;
    int (*tests[])() = {testManagedGateway_create, testManagedGateway_setMQTTLogHandler, testManagedGateway_sendEvent, testManagedGateway_connect};
    int i;
