...
/*
 * systemManagedDevice_tests.c: IBM Watson IoT Platform C Client Managed Device API system level tests
 *
 * - Send events
 * - Receive commands
 */
...
int test_sendEvent(void)
{
    ...
    TEST_ASSERT("test_sendEvent: Create managedDevice with valid config", rc == IOTPRC_SUCCESS, "rcE=%d rcA=%d", IOTPRC_SUCCESS, rc);
    ...
    TEST_ASSERT("test_sendEvent: Destroy a valid managedDevice handle", rc == IOTPRC_SUCCESS, "rcE=%d rcA=%d", IOTPRC_SUCCESS, rc);
...
int main(int argc, char** argv)
{
    ...
    testStart("IBM IoT Platform Client: Managed Device system level tests", count);
    ...
    testEnd("IBM IoT Platform Client: Managed Device system level tests", count);
