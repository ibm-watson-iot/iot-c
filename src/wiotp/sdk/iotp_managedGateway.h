...
/** @file iotp_managedGateway.h
    @brief **Managed Gateway Client Reference** exposes functions and declarations to build IoTP managed Gateway client.
...
/**
 * The IoTPManagedGateway_setEventCallback() API sets event callback to get notifications on event delivery status.
 *
 * @param managedGateway - A pointer to IoTP managed gateway handle.
 * @param cb             - A Function pointer to the IoTPEventCallbackHandler.
 * @return IOTPRC        - Returns IOTPRC_SUCCESS onsuccess or IOTPRC_* on error
 */
DLLExport IOTPRC IoTPManagedGateway_setEventCallback(IoTPManagedGateway *managedGateway, IoTPEventCallbackHandler cb);
