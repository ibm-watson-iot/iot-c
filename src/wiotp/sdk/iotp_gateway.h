...
/** @file iotp_gateway.h
    @brief **Gateway Client Reference** exposes functions and declarations to build IoTP gateway client.
...
/**
 * The IoTPGateway_setEventCallback() API sets event callback to get notifications on event delivery status.
 *
 * @param gateway        - A pointer to IoTP gateway handle.
 * @param cb             - A Function pointer to the IoTPEventCallbackHandler.
 * @return IOTPRC        - Returns IOTPRC_SUCCESS onsuccess or IOTPRC_* on error
 */
DLLExport IOTPRC IoTPGateway_setEventCallback(IoTPGateway *gateway, IoTPEventCallbackHandler cb);
