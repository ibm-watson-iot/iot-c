...
/* Set event callback */
IOTPRC IoTPManagedGateway_setEventCallback(IoTPManagedGateway *managedGatway, IoTPEventCallbackHandler cb)
{
    IOTPRC rc = IOTPRC_SUCCESS;

    rc = iotp_client_setEventCallbackHandler((void *)managedGatway, IoTP_Handler_EventCallback, cb);
    if ( rc != IOTPRC_SUCCESS ) {
