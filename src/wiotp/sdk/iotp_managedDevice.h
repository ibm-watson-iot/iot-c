...
/** @file iotp_managedDevice.h
    @brief **Managed Device Client Reference** exposes functions and declarations to build IoTP managed device client.
...
/**
 * The IoTPManagedDevice_setEventCallback() API sets event callback to get notifications on event delivery status.
 *
 * @param managedDevice  - A pointer to IoTP managed device handle.
 * @param cb             - A Function pointer to the IoTPEventCallbackHandler.
 * @return IOTPRC        - Returns IOTPRC_SUCCESS onsuccess or IOTPRC_* on error
 */
DLLExport IOTPRC IoTPManagedDevice_setEventCallback(IoTPManagedDevice *managedDevice, IoTPEventCallbackHandler cb);
