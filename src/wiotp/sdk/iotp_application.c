...
/* Set event callback */
IOTPRC IoTPApplication_setEventCallback(IoTPApplication *application, IoTPEventCallbackHandler cb)
{
    IOTPRC rc = IOTPRC_SUCCESS;

    rc = iotp_client_setEventCallbackHandler((void *)application, IoTP_Handler_EventCallback, cb);
    if ( rc != IOTPRC_SUCCESS ) {
