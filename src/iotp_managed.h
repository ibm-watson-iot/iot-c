...
#if !defined(IOTP_MANAGED_H_)
#define IOTP_MANAGED_H_
...
/**
 * IoTPDevice_setLogHandler: Sets a callback to handle client library async messages and tracing
 *
 * @param device         - A pointer to IoTPDevice handle.
 * 
 ...
 *                       - IoTP_RC_INVALID_HANDLE if handle is not valid
 */
DLLExport IoTP_RC IoTPDevice_setLogHandler(IoTPDevice *device, IoTPLogHandler cb);
